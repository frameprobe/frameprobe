...
- **flicker** — only shown when some clicks were measured through backlight PWM. Displays that dim by strobing the backlight (e.g. MacBook Pro mini-LED, ~3 kHz) turn the lit screen into a full-scale square wave the photodiode captures raw, which corrupts normal crossing detection. Every click is checked before detection: when a reference window oscillates strongly periodically and the oscillation reaches past the midpoint of the swing, detection runs on a moving average of exactly one strobe period, which cancels the strobe — so all clicks from a strobing display are measured the same way, regardless of direction. The line reports how many clicks this applied to and the strobe frequency. Timing is blurred by at most the strobe period (well under 0.5 ms), and the separation figures for these clicks describe the filtered signal. Mild flicker that stays below the midpoint (QD-OLED) does not trigger this and keeps raw detection.
